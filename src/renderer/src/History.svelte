<script lang="ts">
  ...
  let {
    ...
    onedit: (item: { id: number; duration_seconds: number }) => void
...
<div class="mt-6">
  <h2 class="text-xs font-medium text-zinc-400 mb-2 uppercase tracking-wide">Historial</h2>
        ...
        <tbody>
          {#each state.history as item (item.id)}
            <tr class="border-b border-zinc-800/50 hover:bg-zinc-800/30">
              ...
              <td class="py-1.5 pr-2 font-mono text-zinc-200"
                >{formatDuration(item.duration_seconds)}</td
              >
