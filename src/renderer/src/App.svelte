<script lang="ts">
  ...
  let currentTab = $state<'timer' | 'history' | 'settings'>('timer')
  ...
  async function loadSettings(): Promise<void> {
    ...
    setPipStyle(s.pipStyle as 'transparent' | 'dark' | 'light' | 'green' | 'blue' | 'amber' | 'custom')
  ...
  onMount(() => {
    ...
    cleanupSettings = window.electronAPI.onSettingsChanged((data) => {
      const d = data as { pipStyle?: string; customColors?: { bg: string; text: string; border: string } }
      if (d.pipStyle) setPipStyle(d.pipStyle as 'transparent' | 'dark' | 'light' | 'green' | 'blue' | 'amber' | 'custom')
...
{#if isPipMode}
  ...
  <div
    class={'flex flex-col items-center justify-center h-screen select-none rounded-lg' + (getState().pipStyle !== 'custom' && PIP_STYLES[getState().pipStyle] ? ' ' + PIP_STYLES[getState().pipStyle].bg + ' ' + PIP_STYLES[getState().pipStyle].text + ' ' + PIP_STYLES[getState().pipStyle].border + ' border' : '')}
    style={getState().pipStyle === 'custom' ? `background-color: ${getState().customColors.bg}; color: ${getState().customColors.text}; border-color: ${getState().customColors.border}; border-width: 1px;` : ''}
  >
    <div style="-webkit-app-region: drag" class="flex flex-col items-center justify-center w-full h-full px-3 py-2">
      <TimerDisplay pip />

      <div class="flex items-center gap-3 mt-1" style="-webkit-app-region: no-drag" class:opacity-80={getState().pipStyle !== 'custom'}>
        <button
          ...
          style={getState().pipStyle === 'custom' ? `color: ${getState().customColors.text}cc` : 'color: rgba(255,255,255,0.8)'}
        ...
        <button
          ...
          style={getState().pipStyle === 'custom' ? `color: ${getState().customColors.text}88` : 'color: rgba(255,255,255,0.6)'}
          title="Salir de PiP"
        >
  ...
  <div class="min-h-screen bg-zinc-950 text-zinc-100 p-4">
    <header style="-webkit-app-region: drag" class="flex items-center justify-between mb-4">
      <div style="-webkit-app-region: no-drag" class="flex gap-4">
        <button
          onclick={() => { currentTab = 'timer'; loadApps() }}
...
{#if showEditModal && selectedHistoryItem}
  <EditModal
    ...
    onclose={() => { showEditModal = false; selectedHistoryItem = null }}
  />
{/if}
